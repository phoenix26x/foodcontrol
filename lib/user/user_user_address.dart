...
import '../setting.dart';
import 'user_address2.dart';
import 'user_address_detail.dart';

class UserAddressUser extends StatefulWidget {
  final String username;

  const UserAddressUser({super.key, required this.username});

  @override
  State<UserAddressUser> createState() => UserAddressState();
}

class UserAddressState extends State<UserAddressUser> {
  List<Map<String, dynamic>> addressList = [];

  Future<void> deleteLocation(String username, int location_id) async {
    var url =
        'http://$ip/restarant_papai/flutter1/lcoation_delete.php'; // แก้ไข URL เป็น location_delete.php
  ...
  Future<void> _navigateToAddressDetails(
            ...
            child: addressList != null && addressList.isNotEmpty
                ? ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return Dismissible(
                        key: Key(addressList[index]['location_id']
                            .toString()), // ใช้ location_id เป็นคีย์
                        onDismissed: (direction) async {
                          // ทำงานเมื่อถูกลบ
                          var username = widget.username;
                          var locationId = int.parse(
                              addressList[index]['location_id'].toString());
                          await deleteLocation(
                              username, locationId); // เรียกฟังก์ชันลบที่อยู่
                          setState(() {
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => UserAddressEditUser(
